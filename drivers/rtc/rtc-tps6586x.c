...
static int tps6586x_alarm2_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
  ...
  tps6586x_rtc_read_time(dev, &tm);
  rtc_tm_to_time(&tm, &rtc_seconds);

  if ((seconds - rtc_seconds) < 0)
      return -EINVAL;

  /* Save the time offset */
  saved_alarm2_sec = seconds - rtc_seconds;

  printk("alarm2 time offset:%lu\n", saved_alarm2_sec);

  return 0;
}

static int __tps6586x_alarm2_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
    struct tps6586x_rtc *rtc = dev_get_drvdata(dev);
    struct device *tps_dev = to_tps6586x_dev(dev);
    unsigned long seconds;
...
static void tps6586x_rtc_shutdown(struct platform_device *pdev)
{
    ...
    rtc_tm_to_time(&tm, &seconds);
    seconds += saved_alarm2_sec;
    rtc_time_to_tm(seconds, &alrm.time);
    alrm.enabled = 1;

    __tps6586x_alarm2_set_alarm(dev, &alrm);
...
static struct platform_driver tps6586x_rtc_driver = {
    ...
    .shutdown = tps6586x_rtc_shutdown,
};
